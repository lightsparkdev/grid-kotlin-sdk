...
interface CardServiceAsync {
     ...
     * Grid to sign Spark token transactions for that card by completing the delegated-key creation
     * flow with `POST /auth/delegated-keys`. Until an active delegated key exists for the card, the
     * card remains in `state: "PENDING_AUTH"` and cannot transact.
     *
     * New cards start in `state: "PROCESSING"` while the card issuer provisions the card. Cards
     * that require delegated signing authorization move from `PENDING_AUTH` to `PROCESSING` after
