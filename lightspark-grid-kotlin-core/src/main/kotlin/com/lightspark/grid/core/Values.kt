...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw LightsparkGridInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw LightsparkGridInvalidDataException("`$name` is not set")
            is JsonNull -> throw LightsparkGridInvalidDataException("`$name` is null")
            else -> throw LightsparkGridInvalidDataException("`$name` is invalid, received $this")
        }
