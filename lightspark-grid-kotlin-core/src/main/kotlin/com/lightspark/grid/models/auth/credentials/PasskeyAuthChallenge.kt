 ...
 * The `challenge` value is the lowercase hex-encoded SHA-256 digest of the canonical
...
private constructor(
    ...
    /**
     * Base64url-encoded WebAuthn credential identifier for this passkey. Present only for `PASSKEY`
     * authentication credentials. Corresponds to `PublicKeyCredential.rawId`; pass this value as
     * `allowCredentials[].id` when requesting a passkey assertion for this auth method.
     *
    ...
    /**
     * Lowercase hex-encoded SHA-256 digest of the canonical session-creation request body for the
     * pending passkey authentication. Do not base64url-decode this field; pass UTF-8 bytes of the
     * string (for example, `new TextEncoder().encode(challenge)`) as the WebAuthn challenge to
     * `navigator.credentials.get()`. Single-use; a new challenge is issued on the next call to
    ...
    class Builder internal constructor() {
        ...
        internal fun from(passkeyAuthChallenge: PasskeyAuthChallenge) = apply {
            ...
            credentialId = passkeyAuthChallenge.credentialId
        ...
        /**
         * Base64url-encoded WebAuthn credential identifier for this passkey. Present only for
         * `PASSKEY` authentication credentials. Corresponds to `PublicKeyCredential.rawId`; pass
         * this value as `allowCredentials[].id` when requesting a passkey assertion for this auth
         * method.
         */
        ...
        /**
         * Lowercase hex-encoded SHA-256 digest of the canonical session-creation request body for
         * the pending passkey authentication. Do not base64url-decode this field; pass UTF-8 bytes
         * of the string (for example, `new TextEncoder().encode(challenge)`) as the WebAuthn
         * challenge to `navigator.credentials.get()`. Single-use; a new challenge is issued on the
         * next call to `POST /auth/credentials/{id}/challenge`.
         */
    ...
    override fun toString() =
        "PasskeyAuthChallenge{id=$id, accountId=$accountId, createdAt=$createdAt, nickname=$nickname, type=$type, updatedAt=$updatedAt, credentialId=$credentialId, challenge=$challenge, expiresAt=$expiresAt, requestId=$requestId, additionalProperties=$additionalProperties}"
}
