...
private constructor(
    ...
    /**
     * Evidence that the customer accepted Grid agreements, at most one entry per type. Unregulated
     * platforms must record acceptance of the Lightspark End User Terms before initiating
     * customer-scoped transactions; those transactions fail until that consent is recorded.
     * Consents can be supplied during customer creation or in a later customer update.
     *
     * @throws LightsparkGridInvalidDataException if the JSON field has an unexpected type (e.g. if
     *   the server responded with an unexpected value).
     */
    ...
    class Builder internal constructor() {
        ...
        internal fun from(individualCustomerCreateRequest: IndividualCustomerCreateRequest) =
            apply {
                ...
                agreementConsents =
                    individualCustomerCreateRequest.agreementConsents.map { it.toMutableList() }
                annualIncomeRange = individualCustomerCreateRequest.annualIncomeRange
                birthDate = individualCustomerCreateRequest.birthDate
                countryOfIssuance = individualCustomerCreateRequest.countryOfIssuance
                ...
                netWorthRange = individualCustomerCreateRequest.netWorthRange
                pepStatus = individualCustomerCreateRequest.pepStatus
                ...
                purposeOfAccount = individualCustomerCreateRequest.purposeOfAccount
                purposeOfAccountOtherDescription =
                    individualCustomerCreateRequest.purposeOfAccountOtherDescription
                region = individualCustomerCreateRequest.region
                sourceOfFundsCategories =
                    individualCustomerCreateRequest.sourceOfFundsCategories.map {
                        it.toMutableList()
                    }
                sourceOfFundsOtherDescription =
                    individualCustomerCreateRequest.sourceOfFundsOtherDescription
                sourceOfWealthCategories =
                    individualCustomerCreateRequest.sourceOfWealthCategories.map {
                        it.toMutableList()
                    }
                sourceOfWealthOtherDescription =
                    individualCustomerCreateRequest.sourceOfWealthOtherDescription
         ...
         * Unregulated platforms must record acceptance of the Lightspark End User Terms before
         * initiating customer-scoped transactions; those transactions fail until that consent is
         * recorded. Consents can be supplied during customer creation or in a later customer
         * update.
         */
    ...
    override fun toString() =
        "IndividualCustomerCreateRequest{customerType=$customerType, address=$address, agreementConsents=$agreementConsents, annualIncomeRange=$annualIncomeRange, birthDate=$birthDate, countryOfIssuance=$countryOfIssuance, currencies=$currencies, email=$email, endUserTermsConsent=$endUserTermsConsent, expectedMonthlyTransactionCount=$expectedMonthlyTransactionCount, expectedMonthlyTransactionVolume=$expectedMonthlyTransactionVolume, fullName=$fullName, identifier=$identifier, idType=$idType, kycStatus=$kycStatus, nationality=$nationality, netWorthRange=$netWorthRange, pepStatus=$pepStatus, phoneNumber=$phoneNumber, platformCustomerId=$platformCustomerId, purposeOfAccount=$purposeOfAccount, purposeOfAccountOtherDescription=$purposeOfAccountOtherDescription, region=$region, sourceOfFundsCategories=$sourceOfFundsCategories, sourceOfFundsOtherDescription=$sourceOfFundsOtherDescription, sourceOfWealthCategories=$sourceOfWealthCategories, sourceOfWealthOtherDescription=$sourceOfWealthOtherDescription, umaAddress=$umaAddress, additionalProperties=$additionalProperties}"
}
