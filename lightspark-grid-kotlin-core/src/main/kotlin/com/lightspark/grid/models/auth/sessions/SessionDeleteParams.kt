 ...
 *    `204`.
 *
 * Sessions also expire on their own. `404` is returned whenever the `id` does not match an active
 * session — whether the session was never issued, was already revoked by a prior call, or has
 * expired past its `expiresAt`. The response code reflects the resource state, not an error in the
 * client's flow: re-revoking an already-revoked or expired session is safe and idempotent at the
 * user intent level.
 */
...
private constructor(
    ...
    companion object {
        ...
        fun none(): SessionDeleteParams = builder().build()
    ...
    class Builder internal constructor() {
        ...
        internal fun from(sessionDeleteParams: SessionDeleteParams) = apply {
            id = sessionDeleteParams.id
            gridWalletSignature = sessionDeleteParams.gridWalletSignature
            requestId = sessionDeleteParams.requestId
    ...
    override fun toString() =
        "SessionDeleteParams{id=$id, gridWalletSignature=$gridWalletSignature, requestId=$requestId, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams, additionalBodyProperties=$additionalBodyProperties}"
}
